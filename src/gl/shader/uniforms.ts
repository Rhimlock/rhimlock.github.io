...
export function getUniformBlocks(program: WebGLProgram) {
  ...
  const indices = new Array(gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS)).fill(-1).map((_,i) => i);
  if (indices.length > 0) {
    const blockIndices : number[] = gl.getActiveUniforms(program,indices,gl.UNIFORM_BLOCK_INDEX);
    blockIndices.filter(i => i >= 0).forEach(blockIndex => {
...
export function getBlockUniforms(program: WebGLProgram, ubo: UBO) {
  ...
  const indices = gl.getActiveUniformBlockParameter(program, blockIndex, gl.UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES) as number[];
...
function lookupSetter(type: number) : Function {
  switch(type) {
    ...
    default: throw "Could not lookup uniform sett for type " + type;
  }
